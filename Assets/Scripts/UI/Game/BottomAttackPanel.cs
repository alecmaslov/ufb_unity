...
public class BottomAttackPanel : MonoBehaviour
{
    ...
    public void OnFinishDice()
    {
        ...
        if (isVampired)
        {
            totalDiceCount += DiceAreaUpdate.instance.diceData[0].diceCount;
            vampireCount = DiceAreaUpdate.instance.diceData[1].diceCount;
        }
        else
        {
            totalDiceCount += DiceAreaUpdate.instance.GetDiceCount();
        }
    ...
    public void OnFinishEnemy()
    {
        enemyStackImage.gameObject.SetActive(false);
        enemyStackImage.transform.parent.gameObject.SetActive(false);
        int diceCount = DiceAreaUpdate.instance.GetDiceCount();
        EventBus.Publish(
            RoomSendMessageEvent.Create(
